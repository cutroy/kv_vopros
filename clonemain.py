...
city_names = pd.unique(data['Город'])
to_pred = pd.read_csv('public_test.csv')
to_pred.fillna(0, inplace=True)
city_names_pred = pd.unique(to_pred['Город'])
cities_final=[]
for i in city_names:
    cities_final.append(i)
for i in city_names_pred:
    cities_final.append(i)
cities_final = list(set(cities_final))
for i in range(len(cities_final)):
    data['Город']=data['Город'].replace(cities_final[i],i+1)
    to_pred['Город']=to_pred['Город'].replace(cities_final[i], i+1)
X=data[['Широта', 'Долгота', 'Индекс', 'Площадь','Город',
       'Этаж', 'Размер_участка',
...
x_to_pred_all = to_pred[['id','Широта', 'Долгота', 'Индекс', 'Площадь','Город',
       'Этаж', 'Размер_участка', 'Расход_тепла',
       'Кво_вредных_выбросов',
       'Кво_комнат', 'Кво_спален', 'Кво_ванных', 'Кво_фото', 'Нлч_парковки',
       'Нлч_почтового_ящика', 'Нлч_балкона', 'Нлч_террасы', 'Нлч_подвала',
       'Нлч_гаража', 'Нлч_кондиционера', 'Последний_этаж', 'Верхний_этаж']]
x_to_pred = to_pred[['Широта', 'Долгота', 'Индекс', 'Площадь','Город',
       'Этаж', 'Размер_участка',
       'Кво_вредных_выбросов',
       'Кво_комнат', 'Кво_спален', 'Кво_ванных', 'Кво_фото', 'Нлч_парковки',
        'Нлч_балкона', 'Нлч_террасы', 'Нлч_подвала',
